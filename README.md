...
- Adjust zoom, dimensions, map/frame opacity, frame colors and thickness, marker/player size, zone text size, labels, shadow, and combat visibility. Filter group members, quests, discovered wayshrines, locations, and the personal waypoint independently. Quest markers include native search areas and floor/door breadcrumbs.
...
Scrying dig-site search zones appear automatically as filled polygons with the native tracked/untracked colors and border. They refresh once per second, follow zoom and rotation, and clip to either minimap shape. Data comes from the same APIs used by [ESO's world map](https://github.com/esoui/esoui/blob/live/esoui/ingame/map/worldmap.lua), without requiring it to be opened. `tests/minimap_dig_site_regression.lua` covers shared sites, polygon alignment, clipping, tracking changes, other-map visibility, changed borders, invalid data, and completion. All five minimap regression suites pass with Fengari; visual appearance still needs an ESO client check.
...
Enable **Misc > Immersive Teleport** to hide the UI and use the character panel camera during recall animations, with VestigeMirror's Left Composition framing. The setting is account wide and off by default. Travel completion, failure, interruption, movement, blocking, death, or disabling the setting restores the UI and releases the camera. Opening a menu also ends the effect. A timeout provides cleanup if an end event is missed.
